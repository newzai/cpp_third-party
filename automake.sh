...
if [ ! -e .result/yum.ok ]; then
	...
	yum install rsync wget  flex byacc byacc-devel bison bison-devel mysql-devel mysql-libs unixODBC openssh-clients freetds unixODBC-devel libtool openssl-devel gmp-devel libpcap-devel gcc gcc-c++
	if [ $? != 0 ];then
